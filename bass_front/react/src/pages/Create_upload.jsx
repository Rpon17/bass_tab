import { useState } from 'react';
import { useNavigate } from 'react-router-dom';

function CreateUpload() {
  const navigate = useNavigate();
  
  const [formData, setFormData] = useState({
    title: '',
    artist: '',
    file: null 
  });

  const [isLoading, setIsLoading] = useState(false);

  const handleChange = (e) => {
    const { name, value, files } = e.target;
    if (name === 'file') {
      setFormData({ ...formData, file: files[0] });
    } else {
      setFormData({ ...formData, [name]: value });
    }
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (isLoading) return;
    if (!formData.file) {
      alert("업로드할 파일을 선택해 주세요.");
      return;
    }

    setIsLoading(true);

    try {
      const data = new FormData();
      data.append('file', formData.file);
      data.append('title', formData.title);
      data.append('artist', formData.artist);

      const response = await fetch('https://rpon17-bass-project-main.onrender.com/v1/upload/jobs/upload', {
        method: 'POST',
        body: data, 
      });

      if (response.ok) {
        navigate('/waiting'); 
      } else {
        const errorDetail = await response.json();
        alert(`업로드 실패: ${errorDetail.detail || "서버 오류"}`);
      }
    } catch (error) {
      console.error("통신 오류:", error);
      alert("서버 연결에 실패했습니다.");
    } finally {
      setIsLoading(false);
    }
  };

  return (
    <div style={{ padding: '40px 20px', textAlign: 'center', minHeight: '100vh', backgroundColor: '#fff' }}>
      
      {/* 상단 네비게이션 - 스타일 통일 */}
      <div style={{ display: 'flex', justifyContent: 'flex-start', gap: '15px', marginBottom: '30px' }}>
        <button onClick={() => navigate('/home')} style={btnStyle}>🏠 Home</button>
        <button onClick={() => navigate('/create')} style={btnStyle}>⬅️ Back</button>
      </div>

      {/* 제목 - 2.2rem, bold 통일 */}
      <h2 style={{ marginBottom: '50px', fontSize: '2.2rem', fontWeight: 'bold' }}>
        📁 직접 파일 업로드
      </h2>

      <div style={formCardStyle}>
        <p style={{ fontSize: '1.1rem', color: '#555', marginBottom: '30px', lineHeight: '1.5' }}>
          내 컴퓨터의 MP3 또는 WAV 파일을<br/>AI가 분석하여 악보를 생성합니다.
        </p>

        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '25px', textAlign: 'left' }}>
          
          <div>
            <label style={labelStyle}>음원 파일 선택</label>
            <input 
              name="file" 
              type="file"
              accept="audio/*"
              onChange={handleChange} 
              style={{ ...inputStyle, padding: '10px' }} // 파일 선택창은 패딩 살짝 조절
              required 
            />
          </div>

          <div>
            <label style={labelStyle}>노래 제목</label>
            <input 
              name="title" 
              type="text"
              placeholder="노래 제목을 입력하세요"
              value={formData.title} 
              onChange={handleChange} 
              style={inputStyle} 
              required 
            />
          </div>

          <div>
            <label style={labelStyle}>아티스트</label>
            <input 
              name="artist" 
              type="text"
              placeholder="아티스트 이름을 입력하세요"
              value={formData.artist} 
              onChange={handleChange} 
              style={inputStyle} 
              required 
            />
          </div>

          <button 
            type="submit" 
            disabled={isLoading} 
            style={{
              ...submitStyle,
              backgroundColor: isLoading ? '#ccc' : '#000', // 검정색으로 통일
              cursor: isLoading ? 'not-allowed' : 'pointer'
            }}
          >
            {isLoading ? '파일 업로드 중...' : '분석 시작하기'}
          </button>
        </form>
      </div>
    </div>
  );
}

// --- 공통 스타일 객체 ---

const btnStyle = { 
  padding: '8px 15px', 
  cursor: 'pointer', 
  border: '1px solid #000', 
  borderRadius: '8px', 
  backgroundColor: '#f5f5f5', 
  fontSize: '0.9rem',
  fontWeight: 'bold' 
};

const formCardStyle = {
  maxWidth: '500px',
  margin: '0 auto',
  padding: '50px 40px',
  border: '2px solid #000', // 2px 굵은 테두리
  borderRadius: '30px',     // 30px 둥근 모서리
  backgroundColor: '#fff'
};

const labelStyle = { 
  display: 'block', 
  marginBottom: '8px', 
  fontWeight: 'bold', 
  fontSize: '1rem' 
};

const inputStyle = { 
  width: '100%', 
  padding: '15px', 
  boxSizing: 'border-box', 
  border: '1px solid #000', 
  borderRadius: '12px', 
  outline: 'none',
  fontSize: '1rem'
};

const submitStyle = { 
  padding: '18px', 
  color: 'white', 
  border: 'none', 
  borderRadius: '15px', 
  fontWeight: 'bold', 
  fontSize: '1.2rem', 
  marginTop: '10px',
  transition: 'transform 0.2s ease'
};

export default CreateUpload;